...
function applySearchSort(
    ...
    if (sort === "date-desc")
        sorted.sort((a, b) =>
            b.publication.created_at.localeCompare(a.publication.created_at),
        );
    if (sort === "date-asc")
        sorted.sort((a, b) =>
            a.publication.created_at.localeCompare(b.publication.created_at),
        );
    if (sort === "title-asc")
        sorted.sort((a, b) =>
    ...
    if (sort === "title-desc")
        sorted.sort((a, b) =>
            b.publication.title.localeCompare(a.publication.title),
        );
    return sorted;
}
...
export function useSearch() {
        ...
        if (results) {
            setResults((prev) =>
                prev
                    ? {
                          ...prev,
                          items: applySearchSort(prev.items, searchSort),
                      }
                    : null,
            );
        }
    ...
    const onSearch = useCallback(
        (q: string) => {
            ...
            if (trimmed) {
                ...
                mutate(
                    { query: trimmed, includeDeleted },
                    {
                        onSuccess: (data) => {
                            const items = applySearchSort(
                                data.data?.items ?? [],
                                searchSort,
                            );
                            setResults(
                                data.data ? { ...data.data, items } : null,
                            );
                        },
                    },
                );
    ...
    const handleIncludeDeletedChange = useCallback(
        (value: boolean) => {
            ...
            if (submittedQuery) {
                mutate(
                    { query: submittedQuery, includeDeleted: value },
                    {
                        onSuccess: (data) => {
                            const items = applySearchSort(
                                data.data?.items ?? [],

...
router.get("/", (req: Request, res: Response) => {
    if (!vectorStore.isReady()) {
        return sendError(
            res,
            503,
            "Service initialising, please retry in a moment",
            "NOT_READY",
        );
    }
    ...
    const project =
        (req.query.project as string | undefined)?.trim() || undefined;
    const category =
        (req.query.category as string | undefined)?.trim() || undefined;
    const sortParam = req.query.sort as string | undefined;
    const sort: BrowseSort = BROWSE_SORT_VALUES.includes(
        sortParam as BrowseSort,
    )
        ? (sortParam as BrowseSort)
        : "date-desc";

    const { items, total, totalPages } = vectorStore.listPublications({
        project,
        category,
        sort,
        page,

...
export async function semanticSearch(
    ...
    const items = vectorStore.searchByVector(queryVector, includeDeleted);

    // Only cache standard searches — recycle bin results are niche and not worth caching
    if (!includeDeleted) {
        cacheService.store(queryVector, items);
        cacheService.storeString(normalizedQuery, items);
    }

    return { items, cacheHit: false, latencyMs: Date.now() - start };
}

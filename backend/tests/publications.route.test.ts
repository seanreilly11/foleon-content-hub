...
function mockListPublications(opts: {
    ...
    if (opts.sort === "title-asc")
        pubs.sort((a, b) => a.title.localeCompare(b.title));
    if (opts.sort === "title-desc")
        pubs.sort((a, b) => b.title.localeCompare(a.title));
    if (opts.sort === "date-asc")
        pubs.sort((a, b) => a.created_at.localeCompare(b.created_at));
    if (opts.sort === "date-desc")
        pubs.sort((a, b) => b.created_at.localeCompare(a.created_at));
...
describe("GET /api/publications", () => {
    beforeEach(() => {
        ...
        (vectorStore.listPublications as jest.Mock).mockImplementation(
    ...
    it("filters by project", async () => {
        const res = await request(app).get(
            "/api/publications?project=Marketing",
        );
    ...
    it("filters by category", async () => {
        const res = await request(app).get(
            "/api/publications?category=Technical+Guides",
        );
        const items = res.body.data.items;
    ...
    it("passes sort=title-asc to listPublications", async () => {
        const res = await request(app).get(
            "/api/publications?sort=title-asc&limit=100",
        );
        expect(res.status).toBe(200);
    ...
    it("trims whitespace from category param before filtering", async () => {
        await request(app).get(
            "/api/publications?category=%20Technical+Guides%20",
        );
        expect(vectorStore.listPublications).toHaveBeenCalledWith(

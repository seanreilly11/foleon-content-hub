...
describe('CacheService', () => {
  ...
  beforeEach(() => { cacheService = new CacheService(); });
  // ─── Vector cache ─────────────────────────────────────────────────────────
  ...
  describe('vector cache — lookup', () => {
    ...
    it('returns null for expired entries (TTL)', () => {
      const vec = unitVector(4, 0);
      cacheService.store(vec, mockSearchResults);
      (cacheService as unknown as { cache: Array<{ timestamp: number }> })
    ...
    it('stored results are a defensive copy — external mutation does not corrupt cache', () => {
      ...
      expect(cacheService.lookup(unitVector(4, 0))).toEqual(mockSearchResults);
    });
  });
  ...
  describe('string cache — lookupString', () => {
    ...
    it('returns results for an exact string match', () => {
      ...
      expect(cacheService.lookupString('success story')).toEqual(mockSearchResults);
    });
    ...
    it('returns null for expired entries (TTL)', () => {
      ...
      const stringCache = (cacheService as unknown as {
    ...
    it('stored results are a defensive copy — external mutation does not corrupt cache', () => {
      ...
      results.splice(0);
      expect(cacheService.lookupString('query')).toEqual(mockSearchResults);
    });
  });
  ...
  describe('string cache — storeString', () => {
    ...
    it('evicts oldest entry (FIFO) when MAX_CACHE_SIZE is reached', () => {
      ...
      expect(cacheService.lookupString('new-query')).toEqual(mockSearchResults);
    });
    ...
    it('purges all expired entries before checking capacity', () => {
      ...
      expect(cacheService.lookupString('fresh-query')).toEqual(mockSearchResults);
      expect(cacheService.size()).toBe(1);
    });
  });

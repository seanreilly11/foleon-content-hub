...
function setupMocks() {
    (openai.chat.completions.create as jest.Mock)
        .mockResolvedValueOnce({
            choices: [{ message: { content: JSON.stringify(mockPass1) } }],
        })
        .mockResolvedValueOnce({
            choices: [{ message: { content: JSON.stringify(mockPass2) } }],
        });
}
    ...
    describe("API call strategy", () => {
        ...
        it("Pass 1 sends only unique project_name values (no duplicates)", async () => {
            ...
            const pass1Body = JSON.parse(
                (openai.chat.completions.create as jest.Mock).mock.calls[0][0]
                    .messages[1].content,
            );
